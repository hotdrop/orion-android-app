...
- アプリ起動時に、生体認証または端末のPIN・パターン・パスワードを求めます。同じ起動セッション内では、バックグラウンドから復帰しても再認証を求めません。
...
- GoogleドキュメントまたはWord文書（`.docx`）のタイトル
- Google Drive上の最終更新日時
- 設定したフォルダからの相対パス

タイトルが長い場合は最大2行、パスが長い場合は最大1行で省略表示します。文書の本文や概要は表示しません。

### 操作

- SettingsでGoogleアカウントと対象フォルダを設定します。
- ORIONはIncoming Intelligence画面の`SYNC`操作を実行したときだけ、対象フォルダとそのサブフォルダ内のGoogleドキュメントとWord文書（`.docx`）を取得します。起動時やバックグラウンドでは自動取得しません。
  - `SYNC`は一覧項目と区別できる画面上部の円形操作として表示し、件数、新着数、最終同期時刻も同じ上部領域で確認できます。
  - 取得したタイトル、更新日、パスはRoomに保存します。
  - 新しいドキュメントと過去のドキュメントは目視でわかるようにUIで表現します。
- 同期が途中で失敗した場合はRoomの既存一覧を変更せず、対象フォルダ全体を取得できた場合だけキャッシュを更新します。
- 一覧の項目をタップすると、対象文書を対応アプリまたはブラウザで開きます。
- ORION内に文書を表示する画面は作りません。
...
通常の待機中は状態パネルを常設せず、同期中、未設定、オフライン、エラーなど対応が必要な状態だけ通知します。
...
- Google Identity ServicesでGoogleアカウントを認可した後、ORION内のフォルダ選択画面でDriveのフォルダ階層を移動し、現在表示しているフォルダを明示的に選択します。
- フォルダ選択画面はGoogle Drive APIからフォルダのメタデータだけを取得し、フォルダIDと表示名をRoomへ保存します。Google Pickerは使用しません。
...
- アプリ起動時のみの端末認証、セキュアブート、解除演出を実装済みです。
